 ...
 * Includes a "Podium" design for the top 3 and a list for others.
 * * FIX: Removed problematic 'react-redux' module imports and 'RootState' type 
 * imports to resolve the build error. Used window-level hook resolution.
 */
const Leaderboard: React.FC = () => {
  ...
  // Access state with flexible typing to ensure runtime stability
    ...
    <div className="max-w-5xl mx-auto py-16 px-6 space-y-16">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 font-medium text-lg max-w-lg mx-auto">
          Recognizing the visionaries and decision-makers of our community.
        </p>
      </div>

      {/* Podium for Top 3 */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end px-4 max-w-4xl mx-auto">
        {sortedUsers.slice(0, 3).map((user, index) => {
          // Determine podium order for desktop: 2nd (index 1), 1st (index 0), 3rd (index 2)
            ...
            <div key={user.id} className={`flex flex-col items-center space-y-6 ${order}`}>
              <div className="relative group">
                ...
                <img 
                  src={user.avatarURL} 
                  alt={user.name} 
                  className="relative w-28 h-28 rounded-[2rem] border-4 border-white shadow-2xl object-cover bg-slate-100 transition-transform duration-500 group-hover:scale-110"
                  onError={(e) => {
              ...
              <div className={`w-full ${height} bg-white rounded-t-[3rem] shadow-2xl border-x border-t border-slate-100 p-8 flex flex-col items-center justify-between text-center relative overflow-hidden group`}>
                ...
                <div className="space-y-0">
                  <div className="text-5xl font-black text-slate-900 tracking-tighter italic">{user.total}</div>
                  <div className="text-[9px] font-black text-indigo-500 uppercase tracking-[0.3em]">Total Pts</div>
                </div>
      ...
      {sortedUsers.length > 3 && (
        <div className="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden max-w-4xl mx-auto">
                ...
                {sortedUsers.slice(3).map((user, index) => (
                  <tr key={user.id} className="group hover:bg-indigo-50/30 transition-colors">
                    <td className="px-10 py-6">
                      <span className="font-black text-slate-300 group-hover:text-indigo-400 transition-colors text-lg italic">
                        #{index + 4}
                      </span>
                    </td>
                    <td className="px-10 py-6">
                      <div className="flex items-center space-x-5">
                        <img 
                          src={user.avatarURL} 
                          alt="" 
                          className="w-12 h-12 rounded-2xl object-cover shadow-sm bg-slate-100 ring-2 ring-transparent group-hover:ring-indigo-100 transition-all" 
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=6366f1&color=fff`;
                          }}
                        />
